...
export class Level1 extends Phaser.Scene{
    ...
    preload() {
        ...
        this.anims.create({
            ...
            frameRate: 11, 
            repeat: -1,
            frames: this.anims.generateFrameNumbers("SKELLYRANGED", {
        ...
        this.anims.create({
            ...
            frameRate: 11, 
            repeat: -1,
            frames: this.anims.generateFrameNumbers("SPEARSKELLY", {
        ...
        this.load.image("needKey", "./assets/NeedKeyBubble.png");
        this.load.image("WARNING", "./assets/StepByStepBubble.png");
    ...
    create() {
        ...
        this.keyBubble = this.add.image(1180,580, "needKey").setOrigin(0).setDepth(1).setScale(0.1).setVisible(false);
        this.warningBubble = this.add.image(690, 255, "WARNING").setOrigin(0).setDepth(1).setScale(0.1).setVisible(false);
        ...
        this.harun = this.physics.add.sprite(973,278, "idle");
        
        this.harun.setCollideWorldBounds(true);
        


        this.keyboard = this.input.keyboard.addKeys("W, A, D, J, K");
        ...
        this.SkelMelee = this.physics.add.sprite(400,400, "SKELLY");
        this.SkelMelee.body.setSize(120,120,true);
        this.SkelMelee.anims.play("skeleton", true).setScale(.5);

        this.SkelRanged = this.physics.add.sprite(100,200, "SKELLYRANGED");
        this.SkelRanged.body.setSize(120,120,true).setOffset(-45,0);
        this.SkelRanged.anims.play("skeletonRanged", true).setScale(.5);
        this.SkelRanged.flipX = true;

        this.SpearSkelly = this.physics.add.sprite(194,160, "SPEARSKELLY");
        this.SpearSkelly.body.setSize(120,120,true);
        this.SpearSkelly.anims.play("spearSkelly", true).setScale(.5);
       ...
       // skel.create(400, 400, "skeleton");
        ...
        var key = this.physics.add.staticGroup();
        key.create(679,288, "key").setScale(0.05).refreshBody();
      ...
      //  this.path = new Phaser.Curves.Line(10, 20, 1,   )
       ...
       // platforms.create(110,385, 'ground2').setScale(20,4).refreshBody().setAlpha(0);//
        ...
        this.physics.add.collider(this.harun, this.SkelMelee);
        this.physics.add.collider(this.harun, this.SkelRanged);
        this.physics.add.collider(this.harun, this.SpearSkelly);
        this.physics.add.overlap(this.harun, saw, this.sawHit, null, this);


        this.physics.add.collider(this.SkelMelee, platforms);
        this.physics.add.collider(this.SkelRanged, platforms);
        this.physics.add.collider(this.SpearSkelly, platforms);
        this.physics.add.overlap(this.harun, this.SkelMelee, this.meleeAttackOnMeele, null, this);
        this.physics.add.overlap(this.harun, this.SkelRanged, this.meleeAttackOnRanged, null, this);
        this.physics.add.overlap(this.harun, this.SpearSkelly, this.meleeAttackOnSpear, null, this);
        this.physics.add.overlap(this.harun, key,this.getKey, null, this);

        var tween = this.tweens.add({
            targets: this.SpearSkelly,
            x: 647,
            //ease: 'Power1',
            duration: 5000,
            yoyo: true,
    ...
    onYoyoHandler (tween, target)
{
  ...
  //  this.harun.disableBody(false, false);
    ...
    meleeAttackOnMeele(harun, SkelMelee){
        if(this.keyboard.J.isDown){
            this.sound.play("DemonDeath");
            this.SkelMelee.disableBody(true,true);
        }else {
            this.sound.play("Dead");
            this.harun.x = 25;
            this.harun.y = 550;
    ...
    meleeAttackOnRanged(harun, SkelRanged){
        if(this.keyboard.J.isDown){
            this.sound.play("DemonDeath");
            this.SkelRanged.disableBody(true,true);
        }else {
            this.sound.play("Dead");
            this.harun.x = 25;
            this.harun.y = 550;
    ...
    meleeAttackOnSpear(harun, SpearSkelly){
        if(this.keyboard.J.isDown){
            this.sound.play("DemonDeath");
            this.SpearSkelly.disableBody(true,true);
        }else {
            this.sound.play("Dead");
            this.harun.x = 25;
            this.harun.y = 550;
    ...
    update(delta) {
        ...
        if(this.keyboard.D.isDown ){
            ...
            console.log("Key Collected = " + this.collected);
      ...
      if(this.keyboard.W.isDown && this.harun.body.touching.down){
        ...
        if(this.harun.x > 1153 && this.harun.x < 1263){
            if(this.collected) {
                this.sound.play("keyAllowed");
                this.harun.setVelocityY(-250);
                this.keyBubble.setVisible(false);
            }else {
                this.sound.play("keyNotCollected");
                this.harun.setVelocityY(0);
                this.keyBubble.setVisible(true);
                this.keyBubble.x = this.harun.x + 10;
                console.log("I need a key");
            }
      ...
      if(this.harun.x >= 668 && this.harun.x <= 732 && this.harun.y===399){
          this.warningBubble.setVisible(true);
          this.warningBubble.x = this.harun.x + 10;
      }else {
...
/*
    SpeakSkellyMove(){
        this.SpearSkelly = this.add.tween(this.SpearSkelly).to({
            this.SpearSkelly.x : this.SpearSkelly.x + 50
